...
export async function POST(
  ...
  try {
    ...
    if (!backendResponse.ok) {
      console.error('[Reject User API] 백엔드 응답 오류:', backendResponse.status, backendResponse.statusText);
      const errorText = await backendResponse.text();
      console.error('[Reject User API] 오류 상세:', errorText);
    ...
    console.error('[Reject User API] 사용자 거절 오류:', error);
    return NextResponse.json(
      { error: '서버 오류가 발생했습니다.' },
      { status: 500 }
    );
