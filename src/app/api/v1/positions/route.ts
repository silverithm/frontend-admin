...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!backendResponse.ok) {
      console.error(`[Positions API] 백엔드 응답 오류: ${backendResponse.status}`);
    ...
    console.error('[Positions API] GET 오류:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendResponse = await fetch(
      `${BACKEND_URL}/api/v1/positions?companyId=${companyId}`,
      { method: 'POST', headers: backendHeaders, body: JSON.stringify(body) }
    );

    if (!backendResponse.ok) {
      console.error(`[Positions API] POST 백엔드 응답 오류: ${backendResponse.status}`);
      return NextResponse.json({ error: `백엔드 서버 오류: ${backendResponse.status}` }, { status: backendResponse.status, headers });
    }
    ...
    console.error('[Positions API] POST 오류:', error);
    return NextResponse.json({ error: '서버 내부 오류가 발생했습니다.' }, { status: 500, headers });
  }
}
