...
export async function PUT(
  ...
  try {
    ...
    const backendResponse = await fetch(
      `${BACKEND_URL}/api/v1/positions/${id}`,
      { method: 'PUT', headers: backendHeaders, body: JSON.stringify(body) }
    );

    if (!backendResponse.ok) {
      console.error(`[Position Detail API] PUT 백엔드 응답 오류: ${backendResponse.status}`);
    ...
    console.error('[Position Detail API] PUT 오류:', error);
...
export async function DELETE(
  ...
  try {
    ...
    if (!backendResponse.ok) {
      console.error(`[Position Detail API] DELETE 백엔드 응답 오류: ${backendResponse.status}`);
      return NextResponse.json({ error: `백엔드 서버 오류: ${backendResponse.status}` }, { status: backendResponse.status, headers });
    }
    ...
    console.error('[Position Detail API] DELETE 오류:', error);
    return NextResponse.json({ error: '서버 내부 오류가 발생했습니다.' }, { status: 500, headers });
  }
}
