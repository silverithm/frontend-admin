...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!companyId || !date) {
      return NextResponse.json({ error: 'companyId, date 파라미터가 필요합니다.' }, { status: 400, headers });
    }
    ...
    const backendResponse = await fetch(
      `${BACKEND_URL}/api/v1/attendance/employee?companyId=${companyId}&date=${date}`,
      {
        method: 'GET',
        headers: {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendResponse = await fetch(
      `${BACKEND_URL}/api/v1/attendance/employee?companyId=${companyId}`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
