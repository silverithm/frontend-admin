...
// 그날 하루치 배차 수정본 API 프록시 (조회·기간조회·저장·되돌리기)
...
async function proxy(request: NextRequest, path: string[]) {
  try {
    ...
    const target = `${BACKEND_URL}/api/v1/dispatch-overrides${suffix}${url.search}`;
    ...
    if (!backendResponse.ok) {
      console.error(`[배차 수정본 API] 백엔드 응답 오류: ${backendResponse.status}`);
      return NextResponse.json(data, { status: backendResponse.status, headers });
    }
    return NextResponse.json(data, { headers });
  } catch (error) {
    console.error('[배차 수정본 API] 오류:', error);
