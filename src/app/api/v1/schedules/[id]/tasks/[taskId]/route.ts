...
export async function PUT(
  ...
  try {
    ...
    return relay(await fetch(
      `${BACKEND_URL}/api/v1/schedules/${id}/tasks/${taskId}?companyId=${companyId}`,
      { method: 'PUT', headers: authHeaders(request), body: JSON.stringify(body) }
    ));
  } catch (error) {
    console.error('[Schedule Task API] PUT 오류:', error);
...
export async function DELETE(
  ...
  try {
    ...
    return relay(await fetch(
      `${BACKEND_URL}/api/v1/schedules/${id}/tasks/${taskId}?companyId=${companyId}`,
      { method: 'DELETE', headers: authHeaders(request) }
    ));
  } catch (error) {
    console.error('[Schedule Task API] DELETE 오류:', error);
    return NextResponse.json({ error: '서버 내부 오류가 발생했습니다.' }, { status: 500, headers });
  }
}
