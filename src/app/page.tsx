 ...
 *
 * 카피는 현재 제품 범위(근무조정·월간일정·전자결재·커뮤니티·채팅·공지·직원 앱)를 기준으로 한다.
...
const FEATURES = [
    ...
    },
    {
        ...
        description:
            '직원은 앱에서 휴무 신청, 일정·할 일 확인, 전자결재, 커뮤니티까지 모두 처리합니다. 관리자는 웹에서, 직원은 앱에서 — 각자 편한 곳에서 일합니다.',
    },
];
...
const PREVIEWS = [
    ...
    },
    {
        ...
        height: 1110,
        title: '케어브이 커뮤니티',
        description: '요양 뉴스·게시판·자료실이 한곳에 모입니다',
        alt: '케어브이 커뮤니티 위젯 — 인기글, 요양 소식, 최신글, 새 자료 목록',
    },
];
...
export default function LandingPage() {
    ...
    return (
        <main style={{ minHeight: '100vh', background: 'var(--color-background-surface)' }}>
            ...
            {/* ── 데이터 보안 — "우리 기관 데이터는 어떻게 저장되나요?"에 대한 답 ── */}
            <Section id="security" variant="transparent" padding={0} paddingBlock={10} dividers={['top']}>
                <div style={container(1000)}>
                    <VStack gap={6}>
                        <VStack gap={2} hAlign="center">
                            <Heading level={2} type="display-2" justify="center" textWrap="balance">
                        ...
                        <Grid columns={{ minWidth: 260, repeat: 'fit', max: 2 }} gap={4}>
                            {SECURITY_POINTS.map((point) => (
                                <motion.div key={point.title} {...fadeUp} style={{ height: '100%' }}>
                                    <Card padding={5} height="100%">
                                        <VStack gap={2}>
                                            <HStack gap={2} vAlign="center">
                                            ...
                                            <Text color="secondary">{point.description}</Text>
                                        </VStack>
                                    </Card>
                                </motion.div>
                            ))}
