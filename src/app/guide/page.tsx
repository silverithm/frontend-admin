...
const ADMIN_GUIDE: GuideSection[] = [
        ...
        steps: [
            {
                ...
                items: [
                    ...
                    '양식 파일이 많으면 "대량 양식 업로드"로 폴더째 끌어다 한 번에 등록할 수 있습니다 (파일 하나가 양식 하나)',
    ...
    {
        heading: '대량 문서 업로드 — 쓰던 시스템의 문서 옮겨오기',
        description: '다른 그룹웨어에서 결재가 끝난 문서를 보관용으로 가져옵니다. (기관 관리자만)',
        steps: [
            {
                title: '준비물 — 색인 엑셀과 문서 파일',
                items: [
                    '이관 화면에서 "양식 내려받기"를 눌러 표준 엑셀 양식을 받습니다 (작성요령 시트가 함께 들어 있습니다)',
                    '쓰던 시스템에서 내려받은 문서 목록을 이 양식에 옮겨 채웁니다',
                    '내보낸 파일을 그대로 올려도 됩니다 — 열 이름을 최대한 알아서 맞추고, 못 알아본 열은 화면에 알려드립니다',
                    '기안서 PDF와 첨부파일을 함께 준비합니다 — 엑셀의 파일명과 이름이 같아야 서로 붙습니다',
                ],
            },
            {
                title: '올리기 — 전자결재 > 결재 관리 > 대량 문서 업로드',
                items: [
